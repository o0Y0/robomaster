...
static void imu_cali_slove(INS_Recv_Data_t *recv_data, float const *bmi088, float const *ist8310);
...
void INS_Task(void const * argument)
{
  ...
  for(;;)
  {
			...
			imu_temp_control(bmi088_data[6]);
...
static void INS_Init(INS_Recv_Data_t *recv_data)
{
    bmi088_data = f_get_bmi088_message();
...
static void imu_cali_slove(INS_Recv_Data_t *recv_data, float const *bmi088, float const *ist8310)
{
    for (uint8_t i = INS_X_ADDRESS_OFFSET; i <= INS_Z_ADDRESS_OFFSET; i++)
    {
        recv_data->INS_gyro[i] = bmi088[3] * gyro_scale_factor[i][0] + bmi088[4] * gyro_scale_factor[i][1] + bmi088[5] * gyro_scale_factor[i][2] ;
        recv_data->INS_accel[i] = bmi088[0] * accel_scale_factor[i][0] + bmi088[1] * accel_scale_factor[i][1] + bmi088[2] * accel_scale_factor[i][2] ;
...
static void IMU_getValues(INS_Recv_Data_t *recv_data) 
{
    float bmi088_sum[2][3]={0.f,}; //[0]accel [1]gyro
		...
		bmi088_data = f_get_bmi088_message();//循环读取加速计、陀螺仪、温度的值
		...
		for(uint8_t i=INS_X_ADDRESS_OFFSET;i<=INS_Z_ADDRESS_OFFSET;i++)
		{
				...
				BMI088_FIFO[0][i][99] = bmi088_data[i];  
				BMI088_FIFO[1][i][99] = bmi088_data[i+3];
